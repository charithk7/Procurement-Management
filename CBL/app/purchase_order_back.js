biziApp.controller('purchase_orderController', function ($scope,datatables_service) {

    //Default Dates
    var default_start_date=moment().subtract(15, 'days');
    ...
    $('#daterange-btn').daterangepicker(
        {
            ...
            startDate: moment().subtract(15, 'days'),//Set Default Range
            endDate: moment()
        },
    ...
    $scope.month_display_data='All';
    $scope.info_popup=function(data_id,page){

        $scope.contract_bill_id=data_id;
        var data_set={};
        data_set['data_id']=data_id;
        data_set['page_no']=page;

        if($scope.month_display_data!='All') {
            data_set['start_date'] = $scope.month_start_date;
            data_set['end_date'] = $scope.month_end_date;
            data_set['month_name'] = $scope.month_display_data;
        }else{
            data_set['start_date'] = null;
            data_set['end_date'] = null;
            data_set['month_name'] = '';
        }
            ...
            success: function (response) {
                ...
                if (response['code'] == 201) {
                    ...
                    $scope.$apply(function () //to reset dropdown
                    {
                        ...
                        $scope.total_pages =response['data_total']['no_of_pages'];
                        $scope.total_months =response['data_total']['no_of_months'];

                        $scope.months_details =response['months'];
                        ...
                        if (page == null) {
                            // do something
                            $scope.pagination_display_data="All";
    ...
    $scope.change_month_display=function(month_name,month_start){
        $scope.month_display_data=month_name;
        $scope.month_end_date = moment(month_start).endOf("month");
        $scope.month_start_date=month_start;
        if($scope.pagination_display_data!='All'){
    ...
    $scope.changed_data = function(index) {
        //alert(index);
        $scope.pagination_display_data=index+1;
        $scope.info_popup( $scope.contract_bill_id,index+1);
    };
